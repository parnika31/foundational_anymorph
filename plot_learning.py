...
if __name__ == "__main__":
    ...
    # parser.add_argument("logdir",default="/IdeaProjects/AnyMorph/morphology-baselines/results/cheetahs-universal-0/",type=str)
    ...
    for exp_i, exp_name in enumerate([
        ...
        for method_name, exp_path in zip([
            "Ours-withoutltv","Ours-withltv",
            "anymorph-withoutltv"], [ "/IdeaProjects/AnyMorph/morphology-baselines/results/final_exps_cluster2rainbow/"+ exp_name + "-withoutltv-universal-?/*/metrics.json",
                                  "/IdeaProjects/AnyMorph/morphology-baselines/results/final_exps_cluster2rainbow/" + exp_name + "-n-universal-?/*/metrics.json",
                                  "/IdeaProjects/AnyMorph/morphology-baselines/results/final_exps_cluster2rainbow/" + exp_name + "-wl-anymorph-universal-?/*/metrics.json"
                                  ]):

            files = glob.glob(os.path.join(exp_path))
            ...
            for f in files:
                ...
                if result is not None:
                    ...
                    g = '/'.join(f.strip().split('/')[:-1])
                    variants.append(load_json(
                        os.path.join(g, "config.json")))
                        ...
                        for step, r in zip(time_steps, rewards):
                            ...
                            if step < max_time_step:
                                ...
                                if samples_per_bin[step] < max_points:
                                    ...
                                    pandas_df = pandas_df._append(dict(
                                        time_steps=step, 
                                        rewards=r, 
                                        method=method_name,
        ...
        # axis.set_title(pretty("cheetahs_OursActorOnly & anymorph_seed_0".replace("humanoids-hopper",
        #                                        "humanoid-hoppers")),
    ...
    legend = fig.legend(
                        loc="lower center",
                        prop={'size': 24, 'weight':'bold'}, ncol=1)
    ...
    plt.savefig(os.path.join(parent_dir, exp_name+"_final.png"))
