...
RUN apt-get update && apt-get install -y  --allow-unauthenticated --no-install-recommends\
    ...
    libjpeg-dev libpng-dev python3.6 python3-pip python3-setuptools \
    libgtk3.0 libsm6 python3-venv cmake ffmpeg pkg-config \
    qtbase5-dev libqt5opengl5-dev libassimp-dev libpython3-dev\
    libboost-python-dev libtinyxml-dev bash python3-tk \
    wget unzip libosmesa6-dev software-properties-common \
    libopenmpi-dev libglew-dev graphviz graphviz-dev patchelf
#RUN apt update && apt install -y --allow-unauthenticated --no-install-recommends \
...
ARG user
ARG cuda
RUN useradd -d /project -u $uid $user

RUN pip3 install --upgrade pip
RUN pip3 install pymongo
#RUN pip3 install numpy scipy pyyaml matplotlib ruamel.yaml networkx tensorboardX pygraphviz
RUN pip3 install torch==2.0.0 torchvision==0.15.1 --index-url https://download.pytorch.org/whl/cu118
RUN pip3 install numpy scipy pyyaml matplotlib ruamel.yaml networkx tensorboardX pygraphviz
RUN pip3 install torch-scatter -f https://pytorch-geometric.com/whl/torch-2.0.0+cu118.html
RUN pip3 install torch-sparse -f https://pytorch-geometric.com/whl/torch-2.0.0+cu118.html
...
USER $user
RUN mkdir -p /project/.mujoco \
    && wget https://www.roboti.us/download/mjpro150_linux.zip -O mujoco.zip --no-check-certificate \
    && unzip mujoco.zip -d /project/.mujoco \
    && rm mujoco.zip
RUN wget https://www.roboti.us/download/mujoco200_linux.zip -O mujoco.zip --no-check-certificate \
    && unzip mujoco.zip -d /project/.mujoco \
    && rm mujoco.zip
RUN mv /project/.mujoco/mujoco200_linux /project/.mujoco/mujoco200
RUN wget https://www.roboti.us/file/mjkey.txt -P /project/.mujoco/mjkey.txt

ENV LD_LIBRARY_PATH /project/.mujoco/mjpro150/bin:${LD_LIBRARY_PATH}
ENV LD_LIBRARY_PATH /project/.mujoco/mujoco200/bin:${LD_LIBRARY_PATH}
ENV LD_LIBRARY_PATH  /project/.mujoco/mjkey.txt:${LD_LIBRARY_PATH}
RUN pip3 install 'cython<3'
#RUN pip3 install mujoco-py==1.50.1.68
RUN pip3 install mujoco-py==2.0.2.4 --user ##

#RUN pip3 install git+git://github.com/CampagneLaboratory/torchfold@c2ac41b#egg=torchfold # link not working
RUN pip3 install baselines==0.1.5 --user ##
RUN pip3 install torchfold --user

WORKDIR /project/anymorph
ENV PYTHONPATH /project/anymorph:/project/anymorph/modular-rl/src:/project/anymorph/modular-rl
