...
def plot_generalization(args):
    ...
    exp_name = args.expID
    exp_path = os.path.join(DATA_DIR, "final_exps_cluster2rainbow/"+exp_name)
    ...
    envs = [
        utils.makeEnvWrapper(name, seed=args.seed, obs_max_len=None) for name in env_names
    ]
    ...
    method_name = "Ours-withoutltv"
    if args.actor_type == "smp":
        method_name = "Shared Modular Policies"
    if args.actor_type == "transformer":
        method_name = "Amorpheus"

    if os.path.isfile("../../generalization_final.csv"):
        data = pd.read_csv("../../generalization_final.csv")
    else:
    ...
    data.to_csv("../../generalization_final.csv")
