...
class ModularEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self, xml, use_restricted_obs=False):
        self.xml = xml
        self.use_restricted_obs = use_restricted_obs
    ...
    def _get_obs(self):
        def _get_obs_per_limb(b):
            ...
            if b == "torso":
                angle = 0.0 if not self.use_restricted_obs else self.data.qpos[2]
                velocity = 0.0 if not self.use_restricted_obs else self.data.qvel[2]
                joint_range = [0.0, 0.0]
            else:
                ...
                velocity = self.data.qvel[qpos_adr]
                joint_range = np.degrees(
                    self.sim.model.jnt_range[jnt_adr]
                )  # range of current joint, (2,)
                if not self.use_restricted_obs:  # normalize
                    angle = (np.degrees(angle) - joint_range[0]) / (joint_range[1] - joint_range[0])
                joint_range[0] = (180.0 + joint_range[0]) / 360.0
                joint_range[1] = (180.0 + joint_range[1]) / 360.0
            obs = np.concatenate([obs, [angle], joint_range]
                                 if not self.use_restricted_obs else [[angle], [velocity]]) # => 19 values
            return obs
